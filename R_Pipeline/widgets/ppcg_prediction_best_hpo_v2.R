...
verify(config$performance_metric, 'character', lengths = 1, 
       ...
       null_allowed = T) -> config$performance_metric

###### Pick Best Model's Config: ######
if(is.null(config$performance_metric) | is.null(config$input_model_runid)){
  path_hpojson = sprintf("%s/%s/hpo.json", mc$path_prediction, config$input_agent_runid %>% substr(1,8))
  if(!file.exists(path_hpojson)){
    copy_to_local(mc, bucket = 'prediction', id = config$input_agent_runid, files = 'hpo.json')
  }
  jsonlite::read_json(path_hpojson) -> hpo
  mid_best = hpo$best_modelruns[1]
} else {
...
for(test_date in config$dates){
  out = pc %>% list.extract('dataset', 'mode', 'optimise', 'verbose', 'version', 'max_memory_GB', 'timeout')
    ...
    out$model = list(
      classifier = list(
        ...
        parameters = list(
          ...
          seed = as.integer(sequence(config$num_seeds) - 1)
        ),

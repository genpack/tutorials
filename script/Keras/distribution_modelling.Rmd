...
You may have faced with situations where the output label is highly biased or unbalanced (in classification) or in regression, you may observe outliers or unspecific values. For example there might exist cases where you don't have the label value but can only know the it is greater than or lower than a certain value. This is especially the case in sticky project where you want to include samples of customers who have not churned until the current time. Using classic Machine Learning models, you will need to remove lots of training data samples (and unforetunately, the most recent ones) due to uncertainty of the label.

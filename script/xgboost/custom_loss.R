...
param <- list(max_depth=3, eta=1, nthread = 2, verbosity=0, 
...
cvset = list(list(X = agaricus.test$data %>% as.matrix %>% as.data.frame, y = agaricus.test$label))
cvset = NULL
xgb   = CLS.XGBOOST(max_depth=3, eta=1, cv.set = cvset, show_progress = T)
xgb$fit(agaricus.train$data %>% as.matrix %>% as.data.frame, agaricus.train$label)
xgb$performance(agaricus.test$data %>% as.matrix %>% as.data.frame, agaricus.test$label, 'gini')
